 ...
 * primitive set. Modeled on the harness's own memory backend so the storage
 * domain tests run without touching disk. Lives under `tests/`, never `src/`.
 * @module
 */
...
/**
 * Shared media pool. Hand one instance to several backends to simulate
 * reopening the same medium after a "restart".
 */
