...
插件会注册一组**可被 agent 调用的工具**(dsh 没有聊天命令框架;在 agent harness 中,运维入口就是工具)。直接让 agent 查看或驱动进化循环,无需手动调用 API:
...
各模式可用性:五个工具在所有模式下都会注册;`evolve_apply` / `evolve_rollback`
需要 applier(在 `propose` 与 `auto-apply` 模式挂载),`evolve_cycle` 需要 LLM
provider,且在 `observe` 模式下会拒绝执行。
